...
public class FileEncryptedEventListener {

    private final Logger logger = LoggerFactory.getLogger(FileEncryptedEventListener.class);

    private final IFileManagerService encryptJobService;

    public FileEncryptedEventListener(final IFileManagerService encryptJobService) {
        this.encryptJobService = encryptJobService;
    }

    @KafkaListener(
            topics = "${file.encrypted.queue.name}",
    ...
    public void listenArchiveEventQueue(@Header(KafkaHeaders.RECEIVED_MESSAGE_KEY) String key,
        ...
        logger.info("File encrypted event key: {}, data: {}", key, fileEncryptionData);
        encryptJobService.encrypted(key, fileEncryptionData);
